...
for(;;){
		...
		if($line == 1){
			...
			if($line == 3){
				echo "Γράψτε τα στοιχεία του  Άτομου. Μορφή εισαγωγής(id(ακεραιος), sex, profession_code, name_surname, date_of_birth)\n";
				$line = readline();
				list($person_id,$tperson_sex,$person_profession_code, $person_name_surname,$tperson_date_of_birth) = explode(" ", $line, 3);
				$queryObj = insertper($person_id,$tperson_sex,$person_profession_code, $person_name_surname,$tperson_date_of_birth);
				if(!$queryObj)
				{
					echo "Σφάλμα εισαγωγής.\n";
				}
